    ...
    >
      ...
      {cartItems.length === 0 ? (
        ...
        <ul className="w-full flex flex-col py-2.5">
          {cartItems.map((item) => (
            <li key={item.id} className="border-b py-2 px-4 flex justify-between align-center">
                <p className='text-amber-300-500 text-xl'>{item.title}</p>
                <p className='text-#B88E2F-500'>{item.price}</p>
